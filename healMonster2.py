...
class heal2(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.heal = healMonster.HealMonster()
    ...
    def initUI(self):
        ...
        for row in rows:
            self.pocketmon = row[4]
            self.hp = row[5]

        self.usermon = self.pocketmon.split(",")  # 포켓몬 배열
        self.userhp = self.hp.split(",")  # 포켓몬 생명 배열
    ...
    def clickSelect(self):
        self.close()
        self.heal.heal(self.healMonster)
        self.h3 = healMonster3.heal3()

    def clickBtn1(self):
        if self.count == 0 or self.count==1:
            self.healMonster += self.usermon[0]+","
        else:
            print("초과")

    def clickBtn2(self):
        if self.count == 0 or self.count == 1:
            self.healMonster += self.usermon[1]+","
        else:
            print("초과")

    def clickBtn3(self):
        if self.count == 0 or self.count == 1:
            self.healMonster += self.usermon[2]+","
        else:
            print("초과")

    def clickBtn4(self):
        if self.count == 0 or self.count == 1:
            self.healMonster += self.usermon[3]+","
        else:
            print("초과")

    def clickBtn5(self):
        if self.count == 0 or self.count == 1:
            self.healMonster += self.usermon[4]+","
        else:
            print("초과")

    def clickBtn6(self):
        if self.count == 0 or self.count == 1:
            self.healMonster += self.usermon[5]+","
        else:
            print("초과")
